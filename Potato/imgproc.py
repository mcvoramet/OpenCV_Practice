...
DataDir = "C:/Users/acer/Desktop/EIE461_IMG_proc/PlantVillage-Dataset/Potato/Train/Potato___healthy"
...
def hsvfilter(image):
    ...
    #cv2.imshow("Windows", hsvim)
    #cv2.waitKey(0)

    return segim


def flood_fill(hsvim):
    ...
    fillholeim = hsvim | im_fllod_fill_inv
...
def leafsegmented(OriginalImage):
    ...
    # If datasets was potato healthy dont use this function : 
    #fillholeim = flood_fill(hsvsegim)
    bwimage = smoothimg(hsvsegim)
...
if __name__ == "__main__":
    ...
    PotatoFiles = os.listdir(DataDir)

    PotatoImg = []
    for file in PotatoFiles:
        PotatoImg.append(file)

    PotatoImg = cv2.imread(DataDir + "/" + PotatoImg[0])
    rmbgIm = leafsegmented(PotatoImg)
